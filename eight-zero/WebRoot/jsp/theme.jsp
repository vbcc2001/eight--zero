...
function createfckeditor() {
	var oFCKeditor = new FCKeditor('mt.mcontent');
...
function checkCode() {
	...
	if (code.value != null) {
		...
		var url = "tool/image.jsp/?code=" + code.value;
...
function ajax() {
	...
	//设置回调函数
	req.onreadystatechange = callbackForCode_1;
	var url = "gettype_getType.action";
	req.open("get", url, true);
	req.send(null);
	return true;
}
function callbackForCode_1() {
	var state = req.readyState;
	var board = document.getElementById("borad");
	if (state == 4) {	
		borad.innerHTML = req.responseText;
	}
}
</script>
		<script type="text/javascript">
...
function reloadImage() {
	...
	%>
	<body onload="initFocus();createfckeditor();ajax()">
		<form action="mianTitle_IncreaseTheme.action" method="post"
			name="themeform" onsubmit="checkMtitle()">
			<table border="0" width="80%">
				<tr>
					...
					<td>
						<input type="text" name="mt.mtitle" id="mtitle" width="80%">
						&nbsp;&nbsp;&nbsp;&nbsp;匿名用户不能发表评论！
						<a href=""> 登录</a>|
						<a href="">注册</a>
					</td>
				...
				<tr>
					...
					<td>
						<textarea rows="4" cols="60" name="mt.mcontent">沙发空缺中~~~~</textarea>
					</td>
				</tr>
				<tr>
					<td>
					...
					<td>
						<input type="text" id="code" name="code" style="width: 50px;"
							onblur="checkCode()" />
						&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
						<img src="tool/image.jsp" id="identity">
